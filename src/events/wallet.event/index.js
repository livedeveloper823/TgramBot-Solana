...
const showWallet = async (bot, msg, params) => {
  ...
  const message = walletMsg({ address: wallet.address, balance });
  const keyboard = walletKeyboard({ address: wallet.address });
...
const deposit = (bot, msg) => {
  ...
  bot.sendMessage(chatId, depositMsg()).then(async () => {
    bot.sendMessage(chatId, walletAddressMsg(wallet.address), {
      parse_mode: 'HTML',
    });
  });
};
